        ...
        if (name.trim() !== '' && email.trim() !== '' && password.trim() !== '' && confirmPassword.trim() !== '') {
            ...
            if (confirmPassword === password) {
                ...
                if (confirmTerms) {
                    ...
                    alert('É necessário concordar com os termos de segurança.')
                }
            } else {
                alert('Informe a mesma senha que foi passada no campo acima.')
            }
        } else {
...
export function useLoginUser() {
    ...
    const getUser = (login: string, password: string) => {
      const user = listUser.value.find(res => (login === res.email || login === res.name) && (password === res.password))
